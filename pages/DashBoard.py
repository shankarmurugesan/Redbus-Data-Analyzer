...
def least_bus_count_route():
    st.subheader("State's Least Bus Count Route")

    # Establish a connection to the MySQL database
    mydb = create_connection()
    cursor = mydb.cursor()

    # Define the query to get the least bus count route for each state
    query = """
    SELECT states, route_name, COUNT(busname) AS bus_count
    FROM bus_routes
    GROUP BY states, route_name
    HAVING bus_count = (
        SELECT MIN(bus_count)
        FROM (
            SELECT states, route_name, COUNT(busname) AS bus_count
            FROM bus_routes
            GROUP BY states, route_name
        ) AS subquery
        WHERE subquery.states = bus_routes.states
    )
    """
    ...
    # Process the results into a DataFrame
    data = {'state': [], 'route_name': [], 'bus_count': []}
    for row in rows:
        state, route_name, bus_count = row
        data['state'].append(state)
    ...
    sns.barplot(data=df, y='state', x='bus_count', hue='route_name', dodge=False, ax=ax)

    # Adding labels and title
    ax.set_xlabel('Number of Buses')
    ax.set_ylabel('State with Route Name')
    ax.set_title('Least Bus Count Route per State')
    plt.xticks(rotation=45)
