...
def main():
    st.header("North Bengal")
    # Add the content and logic for this page here
    st.subheader("Welcome to the North Bengal NBSRTC page!")

    if st.button('Scrap Data'):
        # Call scrabdata only when the button is clicked
        scrabdata(unique_key="North_Bengal_NBSRTC")
    ...
    if st.session_state.get('page') == "filters":
        filterfunc(unique_key="North_Bengal_NBSRTC")
